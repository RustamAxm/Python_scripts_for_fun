...
class WorkerThread(QThread):
    ...
    def run(self):
        ...
        self.ps.close()
        print(abs(Time-time.clock()), "sec")
        np.savetxt('22022019_15K_0.0400Tesla_ac200_840nm_2.txt', np.column_stack((A[:, 0], Sum)), fmt='%10.4f')
       # np.savetxt('30102018_test_aver_.txt', np.column_stack((A[:, 0], np.convolve(Sum, window(100),'same'))), fmt='%10.4f')
        np.savetxt('Test_Time22022019_15K_0.0400Tesla_ac200_840nm_average_2.txt', np.column_stack((average(A[:, 0],64), average(Sum, 64))), fmt='%10.4f')
        print("Data saved")
