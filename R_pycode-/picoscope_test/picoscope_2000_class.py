...
class Pico2000(ctypes.WinDLL):
    ...
    def __init__(self,serialnumber = None,yrange = 9,timebase = 10,dx = 0,nSegments = 1,bufszPreferred = 1000000):
        ...
        super(Pico2000,self).__init__("ps2000a.dll")
        if not self._handle:
        ...
        self.GetValuesAsync = self.ps2000aGetValuesAsync
        ...
        self._blockReady = BlockReadyCallBackType(self.ps2000aBlockReady)
        self.psRunBlock = self.ps2000aRunBlock
    ...
    def open(self):
        ...
        self.call("ps2000aEnumerateUnits",
        ...
        res = self.ps2000aOpenUnit(ctypes.byref(self.handle),self.serialnumber)
        if res == 3:
            print('No PicoScope 2000 Series device could be found')
            #sys.exit(res)
            return
        elif res == 0:
            print('PicoScope opened successfully. handle is %d' % self.handle.value)
        else:
            print('ps2000aOpenUnit returned %s' % pico_error_string(res))
            #sys.exit(res)
            return
    ...
    def getRange(self):
        mi = ctypes.c_short()
        self.call("ps2000aMinimumValue",self.handle,ctypes.byref(mi))
        ma = ctypes.c_short()
    ...
    def DataReady(self,handle,status,noOfSamples,overflow,pParameter):
        if status:
            print("ps2000aGetValuesAsync returned:",pico_error_string(status))
    ...
    def ps2000aBlockReady(self,handle,status,pParameter):
        if status:
            print("ps2000aRunBlock asynchronously returned:",pico_error_string(status))
        elif self.onBlockReady:
            self.onBlockReady()
        else:
            print ("ps2000aBlockReady got:",handle,status,pParameter)
    def setChannel(self,chan,yrange,offset = 0,enabled = True,coupling = 1):
        res = self.ps2000aSetChannel(self.handle, #short handle,
        ...
        if res == 0:
            ...
            print('ps2000aSetChannel returned %s' % pico_error_string(res))
        self.adjustBufferSize()
    ...
    def setETS(self,cycles,interleave):
        ...
        if res:
            if res ==13:
                print('ps2000aSetEts error: A parameter is not valid.')
            elif res==44:
                print('ETS is set but no trigger has been set. A trigger setting is required for ETS.')
            else:
                print('ps2000aSetEts returned %s' % pico_error_string(res))
            return 0
    ...
    def setSimpleTrigger(self,channel = 0,threshold = 0):
        res = self.ps2000aSetSimpleTrigger(
            self.handle,##short handle,
            1,##short enable,
        ...
        if res:
            print('ps2000aSetSimpleTrigger returned %s' % pico_error_string(res))
            ##sys.exit()
    def setTrigger(self):
...
##} PS2000A_TRIGGER_CHANNEL_PROPERTIES
    ...
    def close(self):
        self.ps2000aCloseUnit(self.handle)
    def runblock(self):
        if self.handle:
            err = self.psRunBlock(
                self.handle, #short handle,
            ...
            if err:
                print('ps2000aRunBlock returned %s' % pico_error_string(err))
            return err
    ...
    def runstreaming(self):
        ...
        if self.handle:
            ...
            return err
    def stop(self):
        res =  self.ps2000aStop(self.handle)
        if res:
            print('ps2000aStop returned %s' % pico_error_string(res))
    ...
    def setChanDatabuffer(self,chan):
        ...
        if err:
            print('ps2000aSetDataBuffer returned %s' % pico_error_string(err))
        else:
    ...
    def getValuesS(self):
        ...
        if err:
            print('ps2000aGetValues returned %s' % pico_error_string(err))
        if overflow.value:
    ...
    def getValuesA(self,segmentIndex = 0):
            ...
            overflow = ctypes.c_short()
            err = self.ps2000aGetValuesAsync(
                self.handle,##short handle,
                0,##unsigned long startIndex,
                ctypes.byref(noOfSamples),##unsigned long * noOfSamples,
                0,##unsigned long downSampleRatio,
    ...
    def setSegments(self,nSegments = 1):
        ...
        self.call("ps2000aMemorySegments",self.handle, nSegments,ctypes.byref(nMaxSample))
...
##            return
    ...
    def getValues(self):
        if not self.streaming:
            ...
            if res:
                print('ps2000aSetDataBuffer returned %s' % pico_error_string(res))

        noOfSamples = ctypes.c_long(self.bufflen)
        overflow = ctypes.c_short()
        res = self.ps2000aGetValues(
            self.handle,##short handle,
            0,##unsigned long startIndex,
            ctypes.byref(noOfSamples),##unsigned long * noOfSamples,
            0,##unsigned long downSampleRatio,
        ...
        if res:
            print('ps2000aGetValues returned %s' % pico_error_string(res))
        if overflow.value:
            print("overflow",overflow.value)
        print('noOfSamples %d' % noOfSamples.value)
        return noOfSamples.value
