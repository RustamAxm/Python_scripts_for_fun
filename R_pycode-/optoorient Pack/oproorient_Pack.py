...
# field = np.array([-2.0, -1.8, -1.6, -1.4, -1.2, -1.0, -0.8, -0.6, -0.4, -0.3, -0.2, -0.1, 0.0, 0.1, 0.2, 0.3, 0.4, 0.6,
...
for i in range(len(field)):
    try:
        ...
        pol = (c2 - c1) / (c2 + c1) * 100
        data = np.array([fld, float(c1), float(c2), float(pol)])
        print(data)
    except:
        traceback.print_exc()
    if i == 0:
    ...
    np.savetxt('PL(B)_temp_1.5K_1432_ex700.txt', Res, fmt='%10.8f',
